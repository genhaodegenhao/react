...
class BuyingPage extends React.Component {
  ...
  componentDidMount() {
    _h5t.track('pageview', {
      eventId: 'H5_financialmarket_p_buyingPage',
    });
    this.props.BuyingPageModel.getAccountInfoLister(this.state.channel);
    this.props.BuyingPageModel.getMinimunAmt(this.state.channel, this.state.productCode);
  }
  // 获取验证码
  getValidateCoder = () => {
    _h5t.track('trackevent', {
      eventId: 'H5_financialmarket_buyGetValidateCode',
    });
    ...
    this.props.BuyingPageModel.getValidateCode(token, sendFlag);
  };
  // 输入金额
  changeInputAmt = (e) => {
    this.props.BuyingPageModel.changeFlag(e);
  };
  // 隐藏浮层
  hideModelMask = () => {
    this.props.BuyingPageModel.hideModelFlag();
  };
  // 勾选协议
  handleChooseDeal = () => {
    this.props.BuyingPageModel.changeChooseDeal();
  };
  // 到充值页面
  handleGoCharge = () => {
    _h5t.track('trackevent', {
      eventId: 'H5_financialmarket_buyRecharge',
    });
    const bankId = this.props.BuyingPageModel.accountInfoObj.bankId;
    this.props.history.push(`/recharge?channel=${bankId}`);
  };
  // 输入验证码
  changeInputCode = (e) => {
    this.props.BuyingPageModel.handlevalidateCodeValue(e);
  };
  handleBuying = () => {
    if (this.props.BuyingPageModel.buyingBtnFlag) {
      _h5t.track('trackevent', {
        eventId: 'H5_financialmarket_buyBuyingBtn',
      });
      this.props.BuyingPageModel.showModelFlag();
    }
  };
  handleAgainBuying = () => {
    const params = {
      ...
      bankName: this.props.BuyingPageModel.accountInfoObj.bankName,
    };
    if (this.props.BuyingPageModel.againBuyingBtnFlag) {
      _h5t.track('trackevent', {
        eventId: 'H5_financialmarket_buyAgainBuyingBtn',
      });
      this.props.BuyingPageModel.confirmBuying(this.state.channel,
        this.state.productCode).then(() => {
        this.props.history.replace(`/buying-success${setUrlParams(params)}`);
      });
    }
  };
      ...
      <div className="buyingPage">
        <div className={s.buyingPageBankName}>
          <div className={s.bankInner}>
            {this.props.BuyingPageModel.accountInfoObj.bankName}
          </div>
        ...
        <div className={s.buyingPagebankInfo}>
          ...
          <div className={s.bankDec}>
            <div className={s.bankInfo}>
              {this.props.BuyingPageModel.accountInfoObj.bankName}
              &nbsp;电子账户(尾号{this.props.BuyingPageModel.pan})
            </div>
            <div className={s.availableBalance}>
              可用余额：
              {this.props.BuyingPageModel.balancePrice}元
            </div>
          </div>
          <div onClick={this.handleGoCharge} className={s.charge}>充值</div>
        </div>
        <div className={s.buyingPagebuyingAmt}>
          <div className={s.buyingAmtTitle}>购买金额(元)</div>
          <div className={s.buyingAmt}>
            <List>
              <InputItem
                clear
                placeholder={this.props.BuyingPageModel.minAmtPlaceHolder}
          ...
          <div className={s.buyingTip}>
            {this.props.BuyingPageModel.interestFlag ?
              <div>预计收益<span>{this.props.BuyingPageModel.ownInterest}</span>元</div> : '当日起息，可随时提现'
            }
          </div>
        </div>
        <div onClick={this.handleChooseDeal} className={s.buyingPagebuyingAgreement}>
          <img className={s.selectIcon} src={`${this.props.BuyingPageModel.chooseDealFlag ? selectIcon : noSelectIcon}`} alt="" />
          <span className={s.buyingAgreement}>确认购买即表示您阅读并接受 <a href="#">《相关协议》</a>并受其约束</span>
        </div>
        <div className={`${this.props.BuyingPageModel.buyingBtnFlag ? s.activeBuyingBtn : s.buyingBtn}`} onClick={this.handleBuying}>确认购买</div>
        ...
        <Modal
          popup
          visible={this.props.BuyingPageModel.showModel}
        ...
        >
          <div className={s.verifyCodeArea}>
            ...
            <div className={s.codeInfo}>
              <div className={s.codeMessage}>短信将发送至{this.props.BuyingPageModel.phone}</div>
              <div className={s.codeMsg}>
                ...
                <Button
                  ...
                  disabled={this.props.BuyingPageModel.sendCodeAbledFlag}
                >
                  {this.props.BuyingPageModel.codeText}
                </Button>
              </div>
            </div>
            <div onClick={this.handleAgainBuying} className={`${this.props.BuyingPageModel.againBuyingBtnFlag ? s.activeBuyingBtnAgain : s.buyingBtnAgain}`}>确认购买</div>
          </div>
        </Modal>
      </div>
    );
