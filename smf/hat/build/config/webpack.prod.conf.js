...
Object.keys(APP_ENTRY_POINT).forEach(name => {
  ...
  webpackConfig.plugins.push(
    new HtmlWebpackPlugin({
      ...
      staticPath: `${PUBLIC_PATH}/${PROJECT_NAME}/assets`,
      timespan: (new Date()).getTime(),
    }),
    new ExtractTextPlugin({
      filename: `${PROJECT_NAME}/assets/css/global.[chunkhash].css`,
      disable: false,
      allChunks: true,
    }),
    new SaveAssetsJson({
      // path: path.join(__dirname, 'dist'),
      filename: `dist/${PROJECT_NAME}/assets/assets.json`,
      prettyPrint: true,
      metadata: {
